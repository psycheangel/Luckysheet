...
const server = {
  ...
  multipleRangeShow: function (id, name, r, c, value) {
    ...
    if ($("#luckysheet-multipleRange-show-" + id).length > 0) {
      ...
      // let itemHtml = '<div id="luckysheet-multipleRange-show-'+ id +'" data-color="'+ luckyColor[_this.multipleIndex] +'" title="'+ name +'" nonce="${luckysheetConfigsetting.cspNonce}" style="position: absolute;left: '+ (col_pre - 1) +'px;width: '+ (col - col_pre - 1) +'px;top: '+ (row_pre - 1) +'px;height: '+ (row - row_pre - 1) +'px;border: 1px solid '+ luckyColor[_this.multipleIndex] +';z-index: 15;">'+
      //                 '<div nonce="${luckysheetConfigsetting.cspNonce}" style="width: 100%;height: 100%;position: absolute;top: 0;right: 0;bottom: 0;left: 0;opacity: 0.03;background-color: '+ luckyColor[_this.multipleIndex] +'"></div>'+
      //                '</div>';
      const uuidInline = getComputedInlineClassStyling(`
        &.layout {
        position: absolute;left: ${col_pre - 1}px;
        width: ${col - col_pre - 1}px;
        top: ${row_pre - 1}px;
        height: ${row - row_pre - 1}px;
        border: 1px solid ${luckyColor[_this.multipleIndex]};
        z-index: 15;
        }
        &.layoutUsername {
        height: 19px;
        ...
        width: max-content;
        position: absolute;
        bottom: ${row - row_pre - 1}px;
        right: 0;
        background-color: ${luckyColor[_this.multipleIndex]};
        color:#ffffff;
        padding:0 10px;
        }
        &.content {
      ...
      let itemHtml = `<div
								...
								class="luckysheet-multipleRange-show ${uuidInline} layout"
								data-color="${luckyColor[_this.multipleIndex]}"
								title="${fullName}">

								<div class="username ${uuidInline} layoutUsername">
								${name}
								</div>

								<div class="${uuidInline} content">
								</div>

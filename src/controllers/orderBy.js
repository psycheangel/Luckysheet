...
export function orderByInitial() {
  ...
  $("#luckysheetselected").click(function (event) {
    ...
    method.createHookFunction("selectorHighlightClicked");
  });
  //排序事件
  ...
  $("#luckysheetorderby").click(function () {
    ...
    if (luckysheet_sort_initial) {
      ...
      `);
      let content = `<div class="luckysheet-sort-modal ${uuidInline} overflow-hidden"><div><label><input type="checkbox" id="luckysheet-sort-haveheader"/><span>${locale_sort.hasTitle}</span></label></div><div id="luckysheet-sort-dialog-tablec" class="${uuidInline} overflow-y-auto"><table data-itemcount="0" cellspacing="0"> <tr><td>${locale_sort.hasTitle} <select name="sort_0"> <option value="1">1</option> <option value="2">2</option> <option value="3">3</option> <option value="4">4</option> </select> </td> <td> <div><label><input value="asc" type="radio" checked="checked" name="sort_0"><span>${locale_sort.asc}A-Z</span></label></div> <div><label><input value="desc" type="radio" name="sort_0"><span>${locale_sort.desc}Z-A</span></label></div></td></tr></table></div><div class="${uuidInline} layout"></div> <div> <span class="luckysheet-sort-dialog-additem ${uuidInline} layout-font">+ ${locale_sort.addOthers}</span> </div> </div>`;
